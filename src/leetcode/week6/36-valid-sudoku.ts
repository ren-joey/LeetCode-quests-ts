...
export const isValidSudoku = (board: string[][]): boolean => {
    // used1: check row
    const used1: number[][] = Array.from({ length: 9 }, () => Array(9).fill(0));
    // used2: check column
    const used2: number[][] = Array.from({ length: 9 }, () => Array(9).fill(0));
    // used3: check sub-box
    const used3: number[][] = Array.from({ length: 9 }, () => Array(9).fill(0));
